...
export default function InventoryControl({ 
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto text-xs text-slate-700">
      
      {/* Sub-tab Switch Controls Panel */}
      <div className="flex border-b border-slate-200 gap-2 select-none">
        <button onClick={() => setInventorySubTab('stock-list')} className={`px-4 py-2 font-medium text-xs rounded-t-xl border-t border-x transition-all ${inventorySubTab === 'stock-list' ? 'bg-white border-slate-200 text-[#5C0612] font-bold' : 'border-transparent text-slate-500 hover:text-slate-900'}`}>
          Active Supply Stock Master
        </button>
        <button onClick={() => setInventorySubTab('add-item')} className={`px-4 py-2 font-medium text-xs rounded-t-xl border-t border-x transition-all ${inventorySubTab === 'add-item' ? 'bg-white border-slate-200 text-[#5C0612] font-bold' : 'border-transparent text-slate-500 hover:text-slate-900'}`}>
          + Catalog New Asset Item
        </button>
        <button onClick={() => setInventorySubTab('history')} className={`px-4 py-2 font-medium text-xs rounded-t-xl border-t border-x transition-all ${inventorySubTab === 'history' ? 'bg-white border-slate-200 text-[#5C0612] font-bold' : 'border-transparent text-slate-500 hover:text-slate-900'}`}>
          Transaction Ledger History
        </button>
      </div>

      {/* VIEW PANEL 1: Stock Master Grid Table Layout */}
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 text-slate-500 border-b border-slate-200 uppercase tracking-wider font-semibold text-[10px]">
                  <th className="p-4">Item Catalog Description</th>
                ...
                {inventoryItems.map((item) => (
                  <React.Fragment key={item.item_id}>
                    <tr className="hover:bg-slate-50/40 font-medium text-xs">
                      ...
                      <td className="p-4 font-sans italic text-slate-600">{item.supplier_donor || 'N/A'}</td>
                      <td className="p-4 font-mono text-slate-500">{item.expiration_date || 'N/A'}</td>
                      <td className="p-4">
                        <div className="flex gap-2 justify-center">
                          <button onClick={() => setSelectedActionItemId(selectedActionItemId === item.item_id ? null : item.item_id)} className="px-3 py-1 bg-white hover:bg-slate-50 border font-semibold rounded-lg text-[11px] shadow-sm">
                            {selectedActionItemId === item.item_id ? 'Close' : 'Adjust Stock'}
                          </button>
                          <button onClick={() => executeInventoryItemPurge(item.item_id)} className="p-1 text-rose-600 hover:bg-rose-50 rounded-lg">✕</button>
                        </div>
                      </td>
                    </tr>
                    ...
                    {selectedActionItemId === item.item_id && (
                      <tr className="bg-slate-50/50">
                        <td colSpan={6} className="p-4 bg-slate-50/80 border-b">
                          <form onSubmit={handleTransactionSubmit} className="grid grid-cols-1 md:grid-cols-5 gap-3 max-w-4xl bg-white p-4 rounded-xl border shadow-inner">
                            <div>
                              <label className="block text-[9px] font-bold text-slate-400 uppercase mb-1">Direction Type</label>
                              <select value={actionForm.transaction_type} onChange={(e) => setActionForm({ ...actionForm, transaction_type: e.target.value })} className="w-full p-1.5 border rounded-lg bg-slate-50"><option value="IN">Stock In (Add)</option><option value="OUT">Stock Out (Deduct)</option></select>
                            </div>
                            <div>
                              <label className="block text-[9px] font-bold text-slate-400 uppercase mb-1">Amount Delta</label>
                              <input type="number" required placeholder="Quantity" value={actionForm.quantity_changed} onChange={(e) => setActionForm({ ...actionForm, quantity_changed: e.target.value })} className="w-full p-1.5 border rounded-lg focus:outline-none" />
                            </div>
                            <div>
                              <label className="block text-[9px] font-bold text-slate-400 uppercase mb-1">Audit Log Reason</label>
                              <input type="text" required placeholder="e.g. Bulk Donation, Feeding" value={actionForm.reason} onChange={(e) => setActionForm({ ...actionForm, reason: e.target.value })} className="w-full p-1.5 border rounded-lg focus:outline-none" />
                            </div>
                            <div>
                              <label className="block text-[9px] font-bold text-slate-400 uppercase mb-1">Supplier / Donor</label>
                              <input type="text" placeholder="Optional source mapping" value={actionForm.supplier_donor} onChange={(e) => setActionForm({ ...actionForm, supplier_donor: e.target.value })} className="w-full p-1.5 border rounded-lg focus:outline-none" />
                            </div>
                            <div className="flex flex-col justify-between">
                              ...
                              <div className="flex gap-2">
                                ...
                                <button type="submit" className="px-3 bg-[#5C0612] text-white border-b-2 border-[#D4AF37] font-bold rounded-lg text-xs">Post</button>
                              </div>
                            </div>
                          </form>
                        </td>
      ...
      {inventorySubTab === 'add-item' && (
        <div className="bg-white border border-slate-200 rounded-2xl p-6 max-w-xl mx-auto shadow-sm animate-fade-in">
          {formMessage.text && (
          ...
          <form onSubmit={handleInventoryCreate} className="space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
              ...
              {/* Dynamic Responsive Layout Grid Allocator based on category state selection */}
              <div className={newInventoryForm.category === 'Food' ? "md:col-span-1" : "md:col-span-2"}>
                <label className="block font-bold text-slate-500 uppercase text-[10px] mb-1">Category</label>
                <select value={newInventoryForm.category} onChange={(e) => setNewInventoryForm({ ...newInventoryForm, category: e.target.value })} className="w-full p-2.5 bg-slate-50 border rounded-xl focus:outline-none font-medium">
              ...
              {/* DATA CONSISTENCY SELECTOR ELEMENT: Dispatches exclusively under Food category properties */}
              {newInventoryForm.category === 'Food' && (
                <div className="md:col-span-1">
                  ...
                  <select value={newInventoryForm.food_type} onChange={(e) => setNewInventoryForm({ ...newInventoryForm, food_type: e.target.value })} className="w-full p-2.5 bg-amber-50/20 border-2 border-[#D4AF37]/40 text-slate-900 font-bold rounded-xl focus:outline-none">
              ...
              <div>
                ...
                <input type="number" required placeholder="0" value={newInventoryForm.quantity} onChange={(e) => setNewInventoryForm({ ...newInventoryForm, quantity: e.target.value })} className="w-full p-2.5 bg-slate-50 border rounded-xl font-mono focus:outline-none" />
              </div>
              
              <div>
                <label className="block font-bold text-slate-500 uppercase text-[10px] mb-1">Unit Type</label>
                <select value={newInventoryForm.unit} onChange={(e) => setNewInventoryForm({ ...newInventoryForm, unit: e.target.value })} className="w-full p-2.5 bg-slate-50 border rounded-xl focus:outline-none font-medium">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              <div>
                ...
                <input type="number" required placeholder="10" value={newInventoryForm.min_threshold} onChange={(e) => setNewInventoryForm({ ...newInventoryForm, min_threshold: e.target.value })} className="w-full p-2.5 bg-slate-50 border rounded-xl font-mono focus:outline-none" />
              </div>
              <div>
                <label className="block font-bold text-slate-500 uppercase text-[10px] mb-1">Expiration Timeline Tracker</label>
                <input type="date" value={newInventoryForm.expiration_date} onChange={(e) => setNewInventoryForm({ ...newInventoryForm, expiration_date: e.target.value })} className="w-full p-2.5 bg-slate-50 border rounded-xl font-mono focus:outline-none" />
              </div>
            </div>
            ...
            <button type="submit" className="w-full py-3 bg-[#5C0612] text-white border-b-4 border-[#D4AF37] rounded-xl font-bold tracking-widest hover:bg-[#42040B] transition-all uppercase shadow-md">
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-50 text-slate-500 border-b border-slate-200 uppercase tracking-wider font-semibold text-[10px]">
                ...
                <th className="p-4">Target Product Component</th>
            ...
            <tbody className="divide-y divide-slate-100 font-mono text-[11px] text-slate-600">
              {transactionLedger.map((tx) => (
                <tr key={tx.transaction_id} className="hover:bg-slate-50/50">
                  <td className="p-4 text-slate-400 text-[10px]">{new Date(tx.logged_at).toLocaleString()}</td>
                  <td className="p-4 font-sans font-bold text-slate-900 text-xs">{tx.inventory?.item_name || 'Scrubbed Asset'}</td>
                  ...
                  <td className="p-4 font-bold text-slate-900">{tx.transaction_type === 'IN' ? '+' : '-'}{tx.quantity_changed} <span className="text-[9px] text-slate-400 font-light font-sans">{tx.inventory?.unit}</span></td>
                  <td className="p-4 font-sans text-slate-700 leading-tight">
                    {tx.reason}
