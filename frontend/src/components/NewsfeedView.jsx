...
export default function NewsfeedView({ session }) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-2 py-4 font-sans antialiased self-start min-h-0">
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 items-start">
        ...
        <div className="lg:col-span-7 space-y-4 w-full">
          ...
          {feedItems.map((item) => {
            const initials = item.author_tag.substring(0, 2).toUpperCase();
            ...
            return (
              <div key={item.feed_id} className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden text-left w-full animate-fade-in">
                ...
                <div className="grid grid-cols-2 border-b border-slate-100 px-2 py-1 bg-white select-none">
                  ...
                  <button className="flex items-center justify-center gap-2 py-2 rounded-md text-slate-600 font-bold text-[13px] hover:bg-slate-100/80 transition-all">
        ...
        {/* ================= RIGHT PANELS: TELEMETRY SIDEBAR CONSOLE ================= */}
        <div className="lg:col-span-5 space-y-4 w-full text-left hidden lg:block sticky top-2">
          
          {/* System Insight Dashboard Block */}
          ...
          {/* MODIFIED: Streamlined to exactly 4 premium, high-impact compliance rules */}
      ...
      {lightboxImg && (
        ...
        >
          <button className="absolute top-6 right-6 text-white/70 hover:text-white font-mono text-xs bg-white/10 hover:bg-white/20 p-2 px-4 rounded-xl transition-all">
            ✕ CLOSE FULLSCREEN
          </button>
          <img 
            src={lightboxImg} 
