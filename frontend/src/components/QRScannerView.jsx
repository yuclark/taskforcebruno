...
export default function QRScannerView({ onProfileIdentified }) {
  // ── CARD 1: QR HARDWARE & OVERRIDE STATE CONTROLS ──
  ...
  // ── CARD 2: AI DESCRIPTIVE SEARCH STATE CONTROLS ──
  ...
  return (
    <div className="w-full min-h-[550px] flex flex-col items-center justify-center relative p-3 sm:p-6 bg-gradient-to-br from-slate-200 via-zinc-200 to-stone-300 rounded-3xl overflow-hidden shadow-inner">
      ...
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-2 gap-6 relative z-10">
        ...
        {/* =========================================================
            CARD 1: HARDWARE OPTICAL SCANNER & OVERRIDE CONSOLE
           ========================================================= */}
        <div className="w-full bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 rounded-[32px] overflow-hidden shadow-2xl border-4 border-[#5C0612] p-5 text-white flex flex-col justify-between relative min-h-[560px]">
          ...
          <div className="flex justify-between items-center text-[10px] opacity-40 font-mono tracking-widest z-10 mb-2">
            <div className="flex items-center gap-1.5">
              <span className={`w-1.5 h-1.5 rounded-full ${isActiveCamera ? 'bg-emerald-500 animate-pulse' : 'bg-amber-500'}`} />
              <span>{isActiveCamera ? 'HARDWARE_LIVE_FEED' : 'HARDWARE_STANDBY'}</span>
            </div>
            <span>LOGIC_v1.4_QR</span>
          </div>

          <div className="my-auto flex flex-col items-center z-10 w-full space-y-4">
            <span className="text-[#D4AF37] text-[10px] uppercase font-bold tracking-widest bg-[#D4AF37]/5 px-2.5 py-0.5 rounded-md border border-[#D4AF37]/10">
              Collar Lens Reader
            </span>
            
            {/* Swiper Viewfinder Element Block */}
            <div className="w-44 h-44 sm:w-48 sm:h-48 rounded-3xl border border-white/10 bg-slate-950 relative flex items-center justify-center overflow-hidden shadow-[inset_0_0_20px_rgba(0,0,0,0.6)] border-b-2 border-[#D4AF37]/20 shrink-0">
          ...
          {/* Terminal Manual Text Field Row Overlay */}
          <div className="bg-slate-950/90 p-3.5 rounded-2xl border border-white/5 shadow-[inset_0_2px_8px_rgba(0,0,0,0.8)] z-10 mt-4 text-left">
        ...
        <div className="w-full bg-gradient-to-b from-slate-950 via-slate-900 to-slate-950 rounded-[32px] overflow-hidden shadow-2xl border-4 border-[#5C0612] p-5 text-white flex flex-col justify-between relative min-h-[560px]">
          ...
          <div className="flex justify-between items-center text-[10px] opacity-40 font-mono tracking-widest z-10 mb-2">
            <div className="flex items-center gap-1.5">
              <span className={`w-1.5 h-1.5 rounded-full ${aiCandidates.length > 0 ? 'bg-emerald-500' : isAiProcessing ? 'bg-amber-500 animate-pulse' : 'bg-slate-600'}`} />
              <span>{aiCandidates.length > 0 ? 'MATRIX_MATCHES_FOUND' : 'AI_PARSER_READY'}</span>
            </div>
            <span>LOGIC_v1.4_AI</span>
          </div>
          ...
          <div className="my-auto flex flex-col justify-start items-center z-10 w-full space-y-4 flex-1 pt-2 overflow-hidden">
            ...
            {/* Candidate Mapping List Displays */}
            {aiCandidates.length > 0 ? (
              <div className="w-full space-y-2 animate-fade-in flex flex-col flex-1 overflow-hidden min-h-0">
                ...
                <div className="space-y-2 overflow-y-auto pr-1 border border-white/5 rounded-2xl p-2 bg-slate-950/60 flex-1">
                  {aiCandidates.map((candidate, idx) => (
                    <div key={candidate.pet_id} className="p-2 border border-white/5 bg-slate-900/50 rounded-xl flex items-center justify-between gap-3 shadow-inner hover:bg-slate-900 transition-colors">
                      <div className="flex items-center gap-2.5 truncate flex-1 text-left">
                        <img src={candidate.primary_image || 'https://images.unsplash.com/photo-1514888286974-6c03e2ca1dba'} alt="" className="w-10 h-10 object-cover rounded-lg border border-white/10 shrink-0 select-none pointer-events-none" />
                      ...
                      <button onClick={() => onProfileIdentified(candidate.pet_id)} className="px-3 py-1.5 bg-[#5C0612] hover:bg-[#42040B] text-white border border-[#D4AF37]/20 text-[9px] font-mono font-bold uppercase rounded-lg shrink-0 transition-all active:scale-95 shadow-md">Load</button>
                    </div>
                  ))}
                </div>
              </div>
            ) : (
              <div className="w-full flex-1 flex flex-col items-center justify-center text-center p-6 border border-dashed border-white/5 rounded-2xl bg-slate-950/30 text-slate-500 italic font-sans text-[11px] min-h-[220px]">
                {isAiProcessing ? (
                  ...
                  <p className="px-4">Awaiting text characteristic token inputs inside the description panel below...</p>
                )}
              </div>
            )}
          ...
          {/* AI Description Input Formulation Console */}
          <div className="bg-slate-950/90 p-3.5 rounded-2xl border border-white/5 shadow-[inset_0_2px_8px_rgba(0,0,0,0.8)] z-10 mt-4 text-left shrink-0">
            ...
            <form onSubmit={handleAISearchSubmit} className="space-y-2 animate-fade-in">
              <textarea rows="2" value={petDescription} onChange={(e) => setPetDescription(e.target.value)} placeholder="Describe animal traits... (Ex: ginger striped cat with white socks, spotted coat askal found near gym)" className="w-full bg-slate-900/60 border border-white/10 rounded-xl p-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-[#D4AF37] font-sans tracking-wide text-white placeholder-slate-600 shadow-inner resize-none leading-relaxed min-w-0" />
